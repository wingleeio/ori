...
- 241bb2d: feat(editor): production-readiness upgrades — links, markdown input rules, heading levels, safer undo, bounded layout memory, block ARIA
...
- 241bb2d: feat(editor): parity batch — markdown import/export, syntax highlighting, block reordering, find & replace, accessible outline
...
- 75c26ba: fix(editor): heading blocks rendered taller than measured, drifting model-positioned overlays
...
- 4a5074d: fix(editor): clicking into an unfocused editor no longer scroll-jumps to the remembered caret
...
- 5329a5d: fix(editor): custom caret no longer draws (misplaced) while a custom block's own control has focus
  ...
  focus directly. Widget contenteditables share the DOCUMENT selection, so the
  branded caret overlay draws inside them identically to the text surface;
  widget form controls (input/textarea) undo the surface's transparent
  `caret-color` (via `--ori-caret`) and show the native caret. The view also
  ignores selectionchange/composition originating inside widget controls, so
  widget-internal editing can never corrupt the model selection.

- Updated dependencies [241bb2d]
- Updated dependencies [241bb2d]
  - @wingleeio/ori-core@0.6.0

## 0.5.2

### Patch Changes

- 2cb26e2: Show the placeholder on the empty block at the caret, not just on a wholly empty document. Pressing Enter onto a new empty paragraph or list item now reveals the placeholder there, positioned at the caret (offset by the block's inset for list/quote/code), and it hides as soon as you type or move to a non-empty block.

## 0.5.1

### Patch Changes

- 63ccc66: Align the empty-document placeholder with the caret. When the only block is an inset block (a to-do/bullet/numbered list item, quote, or code block) the placeholder no longer sits under the marker/checkbox — it now starts where the text and caret do. Adds `EditorController.getBlockInset(id)` to support this.
- Updated dependencies [63ccc66]
  - @wingleeio/ori-core@0.5.1
